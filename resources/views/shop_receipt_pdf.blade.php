    ...
    <div class="table-section bill-tbl w-100 mt-10">
        <table class="table w-100 mt-10">
            ...
            @foreach ($cartItems as $item)
                <tr align="center">
                    <td>{{ $item->product()->withTrashed()->first()->product_name ?? $item->product->product_name }}</td>
                    <td>{{ $item->quantity }}</td>
                </tr>
            @endforeach
            <tr>
                <td colspan="7">
                    <p style="color:#004aad;padding:10px;text-align:center">You get Discount of
                        {{ $user->customer->offer }}
